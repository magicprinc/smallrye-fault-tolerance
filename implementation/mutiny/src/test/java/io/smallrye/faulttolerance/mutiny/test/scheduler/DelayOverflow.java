...
public class DelayOverflow {
    ...
    void scheduleNow(ScheduledThreadPoolExecutor pool, Runnable r, int how) {
        switch (how) {
        case 0:
    ...
    void scheduleAtTheEndOfTime (ScheduledThreadPoolExecutor pool, Runnable r, int how) {
        switch (how) {
        case 0:
    ...
    void test () throws Throwable {
        for (int nowHow = 0; nowHow < 4; nowHow++) {
            for (int thenHow = 0; thenHow < 4; thenHow++) {
                ...
                check(runLatch.await(Utils.LONG_DELAY_MS, MILLISECONDS));
